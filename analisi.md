installiamo le dipendenze

creiamo cartelle varie
nella pagina principale (app.js) creiamo l'instradamento /homepage /books /books/:id con il browserouter



creiamo il layout (header e footer) così che quando cambiamo pagina l'header e il footer rimangono invariati
  
 -HEADER: contiene una nav bar con HOMEPAGE E BOOKS con un logo
 -FOOTER: contiene semplici link ecc

creare l'homepage che visualizza un benvenuto
creare una pagina per la visualizzazione dei libri con funzione di ricerca
crea una pagina per visualizzazione di un singolo film per identificativo 

